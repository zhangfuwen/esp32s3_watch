/**
 * @file watch_face.c
 * @brief Simple Modern Watch Face UI - No Emoji Version
 */
...
#define COLOR_HOUR      lv_color_hex(0x58A6FF)
#define COLOR_MINUTE    lv_color_hex(0xF0F6FC)
#define COLOR_SECOND    lv_color_hex(0xFF7B72)
#define COLOR_DATE      lv_color_hex(0x8B949E)
#define COLOR_BATTERY   lv_color_hex(0x3FB950)
#define COLOR_BAR_BG    lv_color_hex(0x21262D)
...
static void update_battery(void) {
    if (battery_label && battery_bar) {
        // Simple fixed value for now (no driver call)
        int soc = 75;
...
void watch_face_init(void) {
    ...
    ESP_LOGI(TAG, "Creating watch face...");
    ...
    ESP_LOGI(TAG, "Container created");
    
    // === Time Display (Simple Layout) ===
    // Hour
    hour_label = lv_label_create(screen_container);
    ...
    lv_obj_align(hour_label, LV_ALIGN_TOP_MID, -40, 30);
    
    // Colon
    lv_obj_t *colon = lv_label_create(screen_container);
    lv_label_set_text(colon, ":");
    lv_obj_set_style_text_font(colon, &lv_font_montserrat_48, 0);
    lv_obj_set_style_text_color(colon, COLOR_MINUTE, 0);
    lv_obj_align(colon, LV_ALIGN_TOP_MID, 0, 30);
    
    // Minute
    minute_label = lv_label_create(screen_container);
    ...
    lv_obj_align(minute_label, LV_ALIGN_TOP_MID, 40, 30);
    
    // Seconds (smaller)
    second_label = lv_label_create(screen_container);
    lv_label_set_text(second_label, "00");
    lv_obj_set_style_text_font(second_label, &lv_font_montserrat_28, 0);
    lv_obj_set_style_text_color(second_label, COLOR_SECOND, 0);
    lv_obj_align(second_label, LV_ALIGN_TOP_MID, 85, 35);
    
    ESP_LOGI(TAG, "Time labels created");
    ...
    lv_obj_align(weekday_label, LV_ALIGN_CENTER, -30, 10);
    ...
    lv_obj_align(date_label, LV_ALIGN_CENTER, 30, 10);
    
    ESP_LOGI(TAG, "Date labels created");
    
    // === Battery Bar ===
    battery_bar = lv_bar_create(screen_container);
    lv_obj_set_size(battery_bar, 150, 20);
    ...
    ESP_LOGI(TAG, "Battery UI created");
    ...
    ESP_LOGI(TAG, "Watch face init complete");
